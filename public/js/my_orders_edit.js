...
addScreenCardText = async (card_page_no) => {
  ...
  for (let i = 0; i < splitted_text_lines.length; i++) {
    ...
    if (card_page_no == 2 && (i == 1 || i == 2)) {
      ...
      created_elem_string_ec += `<div class="card-details" spellcheck="false" style=" width:max-content; z-index: 1; font-size: ${font_size}px ; ${
        fonts[font_family]
      } color: ${color}; top: ${topMargin}px; ${extra}" >${
        text[card_page_no - 1][i]
      }</div>`;
      created_elem_string_sc += `<div class="card-details" style="width: max-content;  font-size: ${font_size}px ; ${
        fonts[font_family]
      } color: ${color}; top: ${topMargin}px; ${extra}">${
        text[card_page_no - 1][i]
      }</span></div>`;
      ...
      created_elem_string_ec += `<div class="card-details" spellcheck="false" style=" width: max-content; z-index: 1; font-size: ${font_size}px ; ${
        fonts[font_family]
      } color: ${color}; top: ${top_margin}px; ${extra}" >${
        text[card_page_no - 1][i]
      }</div>`;
      created_elem_string_sc += `<div class="card-details" style="width: max-content;font-size: ${font_size}px ; ${
        fonts[font_family]
...
function loadRearrangePopup() {
  for (let i = 1; i <= totalCards; i++) {
    $(`#rpages`).append(`
            <li class="r-li" style="position:relative;" id="rc-${
              cardOrder[i - 1]
            }-container">
                        <div class="r-images" id="rc-${
                          cardOrder[i - 1]
                        }">     </div>
                        <div class="r-pageNo">   <span class="rpageNumber" >Page ${
                          cardOrder[i - 1]
                        }</span>   </div>
                        <div class="delete-images"> <img src="./img/delete_icon.png" id="deleteCard${
                          cardOrder[i - 1]
                        }" onclick="deleteCard(${
      cardOrder[i - 1]
...
async function done_func() {
  var imageList = new Array(totalCards);
  var textList = [];
  var arrayTextList = new Array(totalCards);
  for (let i = 1; i <= totalCards; i++) {
    let selectedItem = document.getElementById(`ec-details-${i}`).querySelectorAll(".card-details");
    let bgimage = document.getElementById(`ec-${i}`).style.backgroundImage;
    bgimage = bgimage.split(`\"`)[1];
    imageList[i - 1] = bgimage;
    var textProp = "";
    arrayTextList[i - 1] = [];
    let ratio = 0.3;
    if (window.innerWidth < 600) ratio = 0.24;
    for (let j = 0; j < selectedItem.length; j++) {
      if (selectedItem[j].innerHTML != "") {
        arrayTextList[i - 1].push(selectedItem[j].innerHTML);
        if (textProp != "") textProp = textProp + "=";
        let textfont =
          selectedItem[j].style.fontFamily +
          selectedItem[j].style.fontWeight +
          selectedItem[j].style.fontStyle;
        ...
        textProp = textProp + ",";
        textProp = textProp + Math.ceil(parseInt(selectedItem[j].style.fontSize.replace("px", "")) / ratio);
        textProp = textProp + ",";
        textProp = textProp + rgb2hex(selectedItem[j].style.color);
        textProp = textProp + ",";
        let x = selectedItem[j].getBoundingClientRect().x - document.getElementById(`ec-details-${i}`).getBoundingClientRect().x;
        let y = selectedItem[j].getBoundingClientRect().y - document.getElementById(`ec-details-${i}`).getBoundingClientRect().y;
        x = Math.ceil(x / ratio);
        y = Math.ceil(y / ratio);
        textProp = textProp + x + "," + y;
        console.log(textProp);
      }
    }
    textList.push(textProp);
  }
  console.log(imageList[0]);
  console.log(textList[0]);
  console.log(arrayTextList[0]);
  let dataObject = new WeddingDataClassForBackend(
    imageList,
    textList,
    arrayTextList,
    true,
  ...
  localStorage.setItem("WeddingDataObjectForBackend",JSON.stringify(dataObject));
  userSessionDataObject = getLocalStorage();
  userSessionDataObject["sectionCardCategory"] = category;
  //   localStorage.setItem(userSessionDataObject, "userSessionDataObject");
  ...
  const observer = pdoc.onSnapshot(
    (docSnapshot) => {
      ...
      localStorage.setItem(
        "card-price",
        JSON.stringify(priceList[`${cardType}Price`].substring(1))
      );
      localStorage.setItem(
        "card-strike-price",
        JSON.stringify(priceList[`${cardType}StrikePrice`].substring(1))
      );
