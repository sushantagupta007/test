  ...
  for (
    ...
    $("#all_wedding_cards_display").append(`
      <div class="col-4 col-sm-4 wedding-card style='height:max-content;'">
        <div class="wedding-card-image" id="wc-${id}" data-id="${id}" onclick="show(this)" style="position: relative; background-size: 100% 100%; display: flex; justify-content: center; align-items: center;">
    ...
    try {
      db.doc(`weddingcards/price`)
        ...
        .then((snapshot) => {
          ...
          price = snapshot.data();
          db.collection(LocationMap[cardCategory][0])
            .get()
            .then((querySnapshot) => {
...
window.addEventListener("load", (event) => {
  ...
  let userSessionDataObject = getLocalStorage();
  cardCategory = userSessionDataObject["sectionCardCategory"];
  if (cardCategory == "royal") {
